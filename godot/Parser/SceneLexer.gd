...
func tokenize(input_text: String) -> Array[Token]:
	...
	while not script.is_at_end_of_file():
		...
		# Пропустить невидимые символы (пробелы, табы уже обработаны)
			...
			# Пропустить неопознанные символы вместо ошибки (для совместимости)
			push_warning("Found unidentified character: %s (unicode: %s) - skipping" % [character, character.unicode_at(0)])
